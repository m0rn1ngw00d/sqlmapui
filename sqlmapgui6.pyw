...
#this shit dont work for multiuser env. Only for a root user only dist like Kali...
#os.path.expanduser("~")
...
#added by wood - common HTTP codes
		...
		self.boxCode['values'] = ('200', '301', '302', '400', '401', '403', '404', '500')
	...
	#--technique=TECH
		...
		self.boxInj['values'] = ('ETQ','BU','SU','BEUS','B','E','U','S','T','Q')
...
# we execute here
	def injectIT(self):
		if (os.name == "posix"):
			cmd = "x-terminal-emulator -hold -e sqlmap" + self.sqlEdit.get()
#			cmd = "/usr/bin/lxterminal -e ./sqlmap.py" + self.sqlEdit.get()
#			cmd = "/usr/bin/xterm -hold -e ./sqlmap.py" + self.sqlEdit.get()
		else:
...
def main():
	...
	s = ttk.Style()
	s.theme_use('default')
	root.title('SQLmap GUI v1.91 - mods by wood')
